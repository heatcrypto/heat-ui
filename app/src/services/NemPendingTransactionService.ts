@Service('nemPendingTransactions')
@Inject('$q', 'http', 'settings')
class NemPendingTransactionsService {
  ...
  readLocalStorage() {
    ...
    for (let i=0; i<window.localStorage.length; i++) {
      ...
      if (key.startsWith('nemPendingTxn:')) {
  ...
  add(address:string, txId:string, timestamp: number) {
    window.localStorage.setItem(`nemPendingTxn:${address}:${txId}:${timestamp}`, "1")
    this.readLocalStorage()
    this.notifyListeners(false)
  }

  remove(address:string, txId:string, timestamp: number) {
    window.localStorage.removeItem(`nemPendingTxn:${address}:${txId}:${timestamp}`)
    this.readLocalStorage()
